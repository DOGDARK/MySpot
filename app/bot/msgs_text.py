from enum import Enum


class MsgsText(Enum):
    GUIDE = (
        """
📂 Категории\n\nПривет! для начала определимся с настроением: выбирайте <b>категорию</b>, что хочется сегодня
""",
        """
⚙️ Фильтры\n\nЕсли хотите, можете выбрать конкретные <b>фильтры</b> — и я найду идеальный вариант специально для вас ✨
""",
        """
🗺️ Геолокация\n\nТак же можете поделиться <b>местоположением</b>, и я покажу лучшие локации рядом 🚶‍♂️📍
""",
    )

    WELCOME = (
        """
<b>Добро пожаловать в Spotts! 😊 </b>

Готовы найти уютное место для отдыха?✨
""",
        """
<b>Привет! Рады видеть вас в Spotts 🌿 </b>

Я помогу подобрать отличные локации. Чем займёмся сегодня?✨
""",
        """
<b>Пора пройтись! 🌞 </b>

Давайте вместе найдём место, которое вам понравится. Выбирайте опцию в меню ☀️
""",
        """
<b>Рады встрече в Spotts 💛 </b>

У меня много интересных мест для вас. С чего начнём?☀️
""",
        """
<b>Приветствуем! 💫 </b>

Хочется отдыха, вдохновения или впечатлений? Я подскажу)🌙
""",
        """
<b>Добро пожаловать! ✨ </b>

Spotts поможет найти подходящее место в пару кликов.💛
""",
        """
<b>Рады видеть вас! 🌸 </b>

Ищете что-то спокойное или активное? Я подскажу! Выбирайте в меню 💫
""",
        """
<b>Добро пожаловать в Spotts 💫 </b>

Здесь вы легко найдёте классные места для отдыха. Начнём? 🔍
""",
        """
<b>Привет-привет! 💛 </b>

Я помогу вам найти лучшие локации рядом или вдали. 🍂
""",
        """
🎉 <b>Добро пожаловать в Myspot!</b>

Я помогу вам найти идеальные места для отдыха по вашим предпочтениям.
""",
    )

    WELCOME_SECOND_PART = """<b>Основные функции:</b>
• 📍 Просмотр мест - смотрите предложения
• 📂 Категории - выберите тип отдыха
• ⚙️ Фильтры - настройте поиск
• 🗺️ Геолокация - ищите места рядом
• ❓ Помощь - получите справку

Выберите действие из меню ниже 👇
"""

    LIKES = """
<b>❤️ Ваши избранные места</b>

<b>Выберите место:</b>
"""

    DISLIKES = """
<b>🚫 Места, которые вам не понравились</b>

<b>Выберите место:</b>
"""

    CATEGORIES_TYPES = [
        "Семейный",
        "С друзьями",
        "Романтический",
        "Активный",
        "Спокойный",
        "Уединённый",
        "Культурный",
        "На воздухе",
    ]

    CATEGORIES = f"""
🎯 <b>Выбор категорий отдыха</b>

Выберите типы отдыха, которые вам интересны. 
Можно выбрать несколько вариантов.

<b>Доступные категории:</b>
• 👨‍👩‍👧‍👦 {CATEGORIES_TYPES[0]} - отдых с детьми и семьей
• 👥 {CATEGORIES_TYPES[1]} - веселое времяпрепровождение в компании  
• 💕 {CATEGORIES_TYPES[2]} - для пар и свиданий
• 🏃‍♂️ {CATEGORIES_TYPES[3]} - спорт и движение
• 🧘‍♂️ {CATEGORIES_TYPES[4]} - расслабление и отдых
• 🌿 {CATEGORIES_TYPES[5]} - тихие места для уединения
• 🎭 {CATEGORIES_TYPES[6]} - музеи, театры, выставки
• 🌳 {CATEGORIES_TYPES[7]} - парки, природа, улица

После выбора нажмите "Подтвердить"
    """

    WISHES_TYPES = [
        "Тусовки",
        "Вкусная еда",
        "Красивый вид",
        "Активность",
        "Развлечения",
        "Расслабление",
        "Музыка",
        "Атмосферность",
        "Творчество",
    ]

    WISHES = f"""
🌟 <b>Выбор пожеланий</b>

Выберите, что для вам важно в месте отдыха. 
Можно выбрать несколько вариантов.

<b>Доступные пожелания:</b>
• 🎉 {WISHES_TYPES[0]} - вечеринки и активное общение
• 🍔 {WISHES_TYPES[1]} - гастрономические удовольствия
• 🌅 {WISHES_TYPES[2]} - живописные места и панорамы
• ⚽ {WISHES_TYPES[3]} - игры и физическая активность
• 🎮 {WISHES_TYPES[4]} - аттракционы и игры
• 😌 {WISHES_TYPES[5]} - релакс и спокойствие
• 🎵 {WISHES_TYPES[6]} - концерты и музыкальные мероприятия
• ✨ {WISHES_TYPES[7]} - особенная атмосфера места
• 🎨 {WISHES_TYPES[8]} - мастер-классы и искусство

После выбора нажмите "Подтвердить"
    """
    HELP_TEXT = """
❓ <b>Помощь по использованию бота</b>

<b>Как пользоваться:</b>
1. Выберите категории отдыха
2. Укажите ваши пожелания
3. Просматривайте подобранные места
4. Используйте фильтры для уточнения

<b>Команды:</b>
/start - перезапустить бота
/help - показать эту справку
        """

    ALL_VIEWED = """
🎉 <b>Все подходящие места просмотрены!</b>

Вы посмотрели все места, которые соответствуют вашим предпочтениям.

Что вы хотите сделать?
• 🔄 Сбросить историю просмотров и начать заново
• ⚙️ Изменить фильтры или категории
• 🗺️ Обновить геолокацию
    """

    VIEW_CHOICE = """
✨ <b>Выберите способ просмотра мест:</b>

• 🗺️ Ближайшие - места рядом с вами, отсортированные по расстоянию
• ⭐ Рекомендации - лучшие места по вашим предпочтениям с указанием расстояния
    """

    NO_PLACES_NEAR = """
❌ <b>Ближайшие места не найдены</b>

Не найдено мест рядом с вами.
Попробуйте изменить ваши категории, пожелания или фильтры.
    """

    NO_PLACES = """
❌ <b>Места не найдены</b>

Не найдено мест, соответствующих вашим предпочтениям.
Попробуйте изменить ваши категории, пожелания или фильтры.
    """

    SEND_LOCATION = """
🗺️ <b>Отправьте ваше местоположение</b>

Пожалуйста, отправьте ваше местоположение, чтобы найти места рядом с вами.

Нажмите на кнопку "📎" (скрепка) внизу и выберите "Местоположение".
    """

    RESET_GEO = """
🗺️ <b>Геолокация сброшена</b>

Ваше местоположение удалено из системы.
    """

    GEO_SAVED = """
✨ <b>Геолокация сохранена!</b>

Ваше местоположение сохранено.
Теперь вы можете искать места рядом с вами.
    """

    NO_FILTERS = """
⚙️ <b>Фильтры поиска</b>

Выберите фильтры, которые хотите применить. 
Можно выбрать несколько вариантов.

<b>Текущие фильтры:</b>
❌ Фильтры не выбраны

После выбора нажмите 'Подтвердить'
    """

    FILTERS = """
⚙️ <b>Фильтры поиска</b>

Выберите фильтры, которые хотите применить. 
Можно выбрать несколько вариантов.

<b>Текущие фильтры:</b>
"""

    SEARCH_FILTER = """
🔍 <b>Поиск фильтра</b>

Введите название фильтра, который хотите найти:
    """

    PROCESSING = """
⏳ <b>Идёт подбор мест по вашим параметрам...</b>

Пожалуйста, подождите немного. Мы выбираем лучшие места, соответствующие вашим фильтрам.
    """


AVAILABLE_FILTERS = [
    "Кафе",
    "Ресторан",
    "Кофейня",
    "Бар",
    "Пиццерия",
    "Суши-бар",
    "Столовая",
    "Паб",
    "Чай с собой",
    "Парк культуры и отдыха",
    "Кинотеатр",
    "Театр",
    "Концертный зал",
    "Музей",
    "Художественная галерея",
    "Выставка",
    "Выставочный центр",
    "Культурный центр",
    "Библиотека",
    "Планетарий",
    "Океанариум",
    "Аквапарк",
    "Бассейн",
    "Каток",
    "Спортивный комплекс",
    "Спортивный клуб",
    "Фитнес-центр",
    "Спортивная школа",
    "Скалодром",
    "Боулинг-клуб",
    "Квесты",
    "Клуб виртуальной реальности",
    "Лазертаг",
    "Пейнтбол",
    "Картинг",
    "Батутный центр",
    "Верёвочный парк",
    "Аттракцион",
    "Парк аттракционов",
    "Детская площадка",
    "Игровая комната",
    "Клуб для детей и подростков",
    "Центр развития ребёнка",
    "Детский лагерь отдыха",
    "Организация и проведение детских праздников",
    "Ночной клуб",
    "Караоке-клуб",
    "Караоке-кабинка",
    "Кальян-бар",
    "Стриптиз-клуб",
    "Банкетный зал",
    "Кейтеринг",
    "Аренда площадок для культурно-массовых мероприятий",
    "Антикафе",
    "Водные прогулки",
    "Пляж",
    "Сауна",
    "Часовня",
    "Смотровая площадка",
    "Сквер",
    "Сад",
    "Лесопарк",
    "Заповедник",
    "Место для пикника",
    "Алкогольные напитки",
    "Рюмочная",
    "Пивоварня",
    "Пивоваренный завод",
    "Сыроварня",
    "Торговый центр",
    "Игорное и развлекательное оборудование",
    "Бильярдный клуб",
    "Игровые приставки",
    "Компьютерный клуб",
    "Киберспорт",
    "Настольные и интеллектуальные игры",
    "Театрально-концертная касса",
    "Концертные и театральные агентства",
    "Горная вершина",
    "Обсерватория",
    "Аэроклуб",
    "Аэротруба",
    "Центр экстремальных видов спорта",
    "Зимние развлечения",
    "Ретритный центр",
    "Декоративный объект",
    "Чайная",
    "Безалкогольный бар",
    "Скейт-парк",
    "Танцплощадка",
    "Оркестр",
    "Тир",
    "Лодочная станция",
    "Водная база",
]
